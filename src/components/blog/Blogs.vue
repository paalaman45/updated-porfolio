          ...
          <article v-for="post in posts" :key="post.id" class="relative isolate flex flex-col gap-8 lg:flex-row">
            ...
            <div>
              <div class="flex items-center gap-x-4 text-xs">
                ...
                <a href="#" class="relative z-10 rounded-full bg-gray-50 px-3 py-1.5 font-medium text-gray-600 hover:bg-gray-100" v-for="tag in post.tags">#{{ tag }}</a>
              </div>
              ...
              <div class="mt-6 flex border-t border-gray-900/5 pt-6">
                <div class="relative flex items-center gap-x-4">
                  <img :src="post.author.profile" alt="" class="size-10 rounded-full bg-gray-50" />
                  <div class="text-sm/6">
...
const getBlogs = async() => {
  ...
  try{
    const response = await axios.get('https://dev.to/api/articles?username=paalaman45');
    const articles = response.data.map(article => ({
      id: article.id,
      title: article.title,
      description: article.description,
      url: article.url,
      published_at: article.readable_publish_date,
      image: article.cover_image ? article.cover_image : 'https://ionicframework.com/docs/img/demos/thumbnail.svg',
      author: {
    ...
    posts.splice(0, posts.length, ...articles);
  }catch(error){
    console.log(error);
  }
}
